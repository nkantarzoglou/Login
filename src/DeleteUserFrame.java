...
public class DeleteUserFrame extends JFrame {
    ...
    public DeleteUserFrame(User_Data user_info) {
        super("Delete user");
        setDefaultCloseOperation(JFrame.EXIT_ON_CLOSE);
        setVisible(true);

        row1 = new JPanel();
        message = new JLabel("Select user to delete twice");
            ...
            public void actionPerformed(ActionEvent e) {
                try {
                    ...
                    if (tffirst.getText().equals("") || tfsecond.getText().equals("")) {
                        ...
                        int updateCount = stmt.executeUpdate("DELETE FROM Users WHERE username = '" + tffirst.getText() + "'");
                        conn.close();
                        if(updateCount == 0){
                            JOptionPane.showMessageDialog(null, "User does not exist", "Info", JOptionPane.INFORMATION_MESSAGE);
                        }
                        else{
                            JOptionPane.showMessageDialog(null, "Deleted user succesfully", "Info", JOptionPane.INFORMATION_MESSAGE);
                        }
